...
My name and age: `r family[8]` `r age['Brian']`

 - What happens when:
   + You extract using a positive index bigger than the vector length?

   **Extracting a positive index larger than the vector returns a result of NA**
   ...
   **Assigning a number to position 20 of the age vector (which before had only 11) created positions from 12 to 20, filling in an NA value for every new value below position 20, and setting postion 20 to be the assigned number.**
   
 - Read carefully the documentation for `is.vector()`. If `is.vector(x)` retrieves `TRUE`, does it mean that `x` is an atomic vector?

  **No, acccording to the help text, is.vector will also produce a TRUE response for a list.**
