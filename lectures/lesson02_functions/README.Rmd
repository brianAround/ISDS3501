...
mean_safe <- function(x) {
  if (!is.double(x)) stop('x must be some kind of numeric vector', call. = F)
  
  mean(x, na.rm = T)
}
mean_safe(c(34L, NA, 2L))
```
...
yearlySalary <- function(x) {
  ...
  working <- ifelse(x >= 8000, x, "NA")
  working <- paste("$", working, sep = '')
  working
}

yearlySalary(c(100, 0, NA, 2000, 8000))
